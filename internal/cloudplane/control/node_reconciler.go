...
func (s *nodeReconciler) reconcilePendingExecutionCapacity(ctx context.Context) (bool, error) {
	...
	if err != nil {
		...
		failErr := s.store.MarkServiceRunFailed(ctx, candidate.ServiceID, reason)
...
func (s *nodeReconciler) reconcileStaleProvisioningNode(ctx context.Context, nodeNamePrefix string, item cloudmodel.Node) error {
	...
	executionErr := s.store.MarkPendingServiceRunFailedForProvisioningNode(ctx, nodeNamePrefix, item.Name, reason)
	return errors.Join(nodeErr, executionErr)
}

func (s *nodeReconciler) reconcileIdleNodes(ctx context.Context) error {
	unsettled, err := s.store.HasUnsettledServiceRuns(ctx)
	if err != nil {
		return err
	}
	if unsettled {
