...
这份文档描述 `mini-cloud` 的真实云端到端验证流程。它不包含 token、云账号密钥、真实资源 ID 等敏感信息。
...
- **release**：构建 control-plane、cloud-plane、node-agent 三个 Linux 二进制和 Web 控制台。
...
当只需要更新 control-plane（例如发布新的 Web 控制台或 control-plane 逻辑）：
...
`e2e` 自动执行 build -> bootstrap -> install -> Web 控制台操作（通过 Playwright）-> update -> smoke 验证 -> destroy 全流程。适合验证代码变更后整个系统仍然闭环可用。
...
- DNS / CDN 生效有延迟，端到端验证内置等待重试逻辑，但云控制台显示可能更慢。
