...
默认配置示例是 `deploy/container/control-plane.yaml.example`。真实部署时，`make deploy` 会把根据 `deploy/ops/config.yaml` 渲染出的 control-plane 配置复制到 `dist/ops/control-plane.yaml`，然后作为 Docker build arg 打进镜像。
